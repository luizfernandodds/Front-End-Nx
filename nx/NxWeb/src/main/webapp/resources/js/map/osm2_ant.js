...
MapController.prototype = {
  ...
  init : function() {
    ...
    styleClass += ".olControlEditingToolbar { top: 60px !important; left: 5px !important; height : 100% !important; width: 27px !important; position: absolute !important; }";
...
//        + "resources/images/map/tools/map_zoom_deactivated.png) !important; background-repeat: no-repeat; background-size: 24px 22px !important; background-position: 0px 0px !important; }";
    ...
    var osmLayer = new OpenLayers.Layer.OSM("Monitoramento",
        "http://119.8.80.198:8080/styles/klokantech-basic/${z}/${x}/${y}.png", {
          layers : 'basic',
          events : {
    ...
    this._Map.addLayers([ osmLayer, googleMaps, googleHybrid ]);
    ...
    var switcher =  new OpenLayers.Control.Button({
      ...
      eventListeners : {
        activate : function() {
          MapController.prototype._Map.setBaseLayer(osmLayer);
...
console.log('name: '+name);
  ...
  addControlPoint : function(value, radius, name) {
	...
	console.log('ADD CONTRO POINT');
    var wkt = this.getWkt(value);
    var vectorLayer = this._Map.getLayersByName("VectorLayer")[0];

    var wktFormat = this.wktFormater();

		console.log('radius DOC: '+document.getElementById(radius).value);

    if (!this.isWkt(value) && !this.isWkt(radius)) {
	
			console.log('addControlPoint IF : ');
	...
	console.log('addControlPoint afterfeaturemodified 1 : ');
	
              document.getElementById(value).value = wktFormat
                  .extractGeometry(args.feature.geometry.getCentroid());

	console.log('addControlPoint afterfeaturemodified 2 : ');

              document.getElementById(radius).value = wkt = parseInt(0.565352 * Math
                  .sqrt(args.feature.geometry.getArea()));

			console.log('addControlPoint ON : ');

console.log('Radius: '+parseInt(0.565352 * Math.sqrt(args.feature.geometry.getArea())));
            }
          });
      editor.activate();
      editor.mode = OpenLayers.Control.ModifyFeature.DRAG;
  ...
  osrmRoute : function(wkt, callback, instructions) {
    ...
    var url = "http://router.project-osrm.org/viaroute?jsonp=function&alt=false&instructions="
        + instructions;
    if ($.type(wkt) == "array") {
      for ( var i in wkt) {
    ...
    $("#" + input.replace(/\:/g, "\\:")).keyup(
        function() {
          ...
          delay(function(el) {
            ...
            $.ajax({
              url : "http://199.217.116.10/nominatim/search",
              data : {
                format : "json",
                q : el.value.toString(),
              },
